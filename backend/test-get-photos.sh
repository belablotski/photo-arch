...
echo "-----------------------------------"
curl -s "${BASE_URL}?author=${AUTHOR}&limit=10" | jq '.'
echo ""
echo ""
...
echo "---------------------"
curl -s "${BASE_URL}?author=${AUTHOR}&limit=5" | jq '.photos | length'
echo ""
echo ""

# Test 3: Filter by date range
echo "Test 3: Filter by date range (2025)"
echo "-----------------------------------"
curl -s "${BASE_URL}?author=${AUTHOR}&dateTaken>=2025-01-01&dateTaken<=2025-12-31" | jq '.photos[].tags.dateTaken'
echo ""
echo ""

# Test 4: Filter by rating
echo "Test 4: Filter by rating >= 4"
echo "------------------------------"
curl -s "${BASE_URL}?author=${AUTHOR}&rating>=4" | jq '.photos[].tags.rating'
echo ""
echo ""

# Test 5: Filter by camera
echo "Test 5: Filter by camera"
echo "------------------------"
curl -s "${BASE_URL}?author=${AUTHOR}&camera=Canon-5D-Mark-IV" | jq '.photos[].tags.camera'
echo ""
echo ""

# Test 6: Filter by GPS
echo "Test 6: Filter by GPS availability"
echo "-----------------------------------"
curl -s "${BASE_URL}?author=${AUTHOR}&hasGPS=true" | jq '.photos[].tags.hasGPS'
echo ""
echo ""
...
echo "-----------------------------"
curl -s "${BASE_URL}?author=${AUTHOR}&customTag1=vacation" | jq '.photos[].tags.customTag1'
echo ""
echo ""
...
echo "--------------------------"
curl -s "${BASE_URL}?author=${AUTHOR}&favorite=true" | jq '.photos[].tags.favorite'
echo ""
echo ""

# Test 9: Combined filters
echo "Test 9: Combined filters (2025 + rating >= 4)"
echo "----------------------------------------------"
